...
export default function ProductPage() {
  ...
  useEffect(() => {
    ...
    if (targetsFromQuery.length > 0) {
      setSelectedTargets(targetsFromQuery);
      handleFilterChange("targets", targetsFromQuery);
    }
  }, [searchParams]);

  // Handle updating the query params when filters or selected targets change
  useEffect(() => {
    ...
    if (filters.pharmaceutical) {
      params.set("type", filters.pharmaceutical);
    }
    selectedTargets.forEach((target) => {
      params.append("target", target);
    });

    router.replace(`?${params.toString()}`, { shallow: true, scroll: false });
  }, [filters.category, filters.pharmaceutical, selectedTargets, router]);

  // Handle toggling targets (add or remove)
  const toggleTarget = (target) => {
    const updatedTargets = selectedTargets.includes(target)
      ? selectedTargets.filter((t) => t !== target)
      : [...selectedTargets, target];

    setSelectedTargets(updatedTargets);
    handleFilterChange("targets", updatedTargets);
  };
