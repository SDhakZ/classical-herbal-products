export const blogData = [
  {
    id: 1,
    title: "Why Shilajit is good for you?",
    slug: "shilajit-benefits",
    author: "Classsical Herbal Team",
    publishedDate: "July 24th, 2024",
    brief:
      "Shilajit, also known as mumiyo or mumie, is an organic mineral compound found in the Himalayan mountains and mountainous regions in Russia, Afghanistan, and elsewhere. This naturally occurring, blackish-brown resin contains over 40 minerals and substances, most notably fulvic acid.",
    image: "shilajit-benefits.png",
    body: [
      {
        title: "4 Benefit of Shilajit",
        description:
          "Shilajit, also known as mumiyo or mumie, is an organic mineral compound found in the Himalayan mountains and mountainous regions in Russia, Afghanistan, and elsewhere. This naturally occurring, blackish-brown resin contains over 40 minerals and substances, most notably fulvic acid.1",
        subBody: [
          {
            title: "1. Has Anti-Inflammatory and Antioxidant Effects",
            description:
              "Shilajit has strong anti-inflammatory and antioxidant effects. The compound is thought to promote greater longevity and have anti-aging properties in Ayurvedic medicine. These benefits can support overall health and reduce the risk of certain conditions.",
            bullet: {
              brief: "Research has shown that shilajit may also:",
              points: [
                "Increase cell turnover",
                "Potentially encourage the body to produce more collagen",
                "Slow down oxidative stress",
              ],
            },
          },
          {
            title: "2. May Prevent Bone Loss",
            image: "bone-loss.png",
            description:
              "Shilajit may have a positive effect on bone health. A study published in 2022 found postmenopausal women with osteopenia (low bone mineral density) who took shilajit supplements daily saw significant improvements in bone density after 11 months. The researchers theorized that this effect may be due to shilajit's ability to speed up cell turnover.",
          },
          {
            title: "3. Could Increase Male Fertility",

            description:
              "Shilajit may increase male fertility by boosting key hormones. Research has shown that taking 500 milligrams (mg) of shilajit per day can significantly increased dehydroepiandrosterone (DHEAS) and total testosterone levels.",
          },
          {
            title: "4. Might Improve Exercise Performance",
            image: "improve-exercise-performance.png",
            description:
              "A growing body of research indicates that shilajit can be helpful for exercise efforts. Some evidence suggests that athletes have less post-workout fatigue and are better able to reach peak muscular strength after taking shilajit supplements.",
          },
        ],
      },
      {
        title: "How to take Shilajit?",
        description:
          "You can find shilajit supplements at many online shops, pharmacies, drugstores, and health food stores. It's available as a dissolvable powder for drinks like water or milk and as a capsule.",
        subBody: [
          {
            title: "Dosage",
            description:
              "Shilajit isn't regulated by the Food and Drug Administration (FDA), meaning there's no set dosage deemed safe and effective. The dose administered in some clinical studies has been between 300–500 mg once per day.2 It's best to follow the dosing instructions from the manufacturer listed on the label if you are taking a shilajit supplement. You can also talk to a healthcare provider for guidance on finding a safe and effective dose.",
          },
        ],
      },
    ],
  },
  {
    id: 2,
    title: "Why Shilajit is good for you?",
    slug: "shilajit-benefits2",
    author: "Classsical Herbal Team",
    publishedDate: "July 24th, 2024",
    brief:
      "Shilajit, also known as mumiyo or mumie, is an organic mineral compound found in the Himalayan mountains and mountainous regions in Russia, Afghanistan, and elsewhere. This naturally occurring, blackish-brown resin contains over 40 minerals and substances, most notably fulvic acid.",
    image: "shilajit-benefits.png",
    body: [
      {
        title: "4 Benefit of Shilajit",
        description:
          "Shilajit, also known as mumiyo or mumie, is an organic mineral compound found in the Himalayan mountains and mountainous regions in Russia, Afghanistan, and elsewhere. This naturally occurring, blackish-brown resin contains over 40 minerals and substances, most notably fulvic acid.1",
        subBody: [
          {
            title: "1. Has Anti-Inflammatory and Antioxidant Effects",
            description:
              "Shilajit has strong anti-inflammatory and antioxidant effects. The compound is thought to promote greater longevity and have anti-aging properties in Ayurvedic medicine. These benefits can support overall health and reduce the risk of certain conditions.",
            bullet: {
              brief: "Research has shown that shilajit may also:",
              points: [
                "Increase cell turnover",
                "Potentially encourage the body to produce more collagen",
                "Slow down oxidative stress",
              ],
            },
          },
          {
            title: "2. May Prevent Bone Loss",
            image: "bone-loss.png",
            description:
              "Shilajit may have a positive effect on bone health. A study published in 2022 found postmenopausal women with osteopenia (low bone mineral density) who took shilajit supplements daily saw significant improvements in bone density after 11 months. The researchers theorized that this effect may be due to shilajit's ability to speed up cell turnover.",
          },
          {
            title: "3. Could Increase Male Fertility",

            description:
              "Shilajit may increase male fertility by boosting key hormones. Research has shown that taking 500 milligrams (mg) of shilajit per day can significantly increased dehydroepiandrosterone (DHEAS) and total testosterone levels.",
          },
          {
            title: "4. Might Improve Exercise Performance",
            image: "improve-exercise-performance.png",
            description:
              "A growing body of research indicates that shilajit can be helpful for exercise efforts. Some evidence suggests that athletes have less post-workout fatigue and are better able to reach peak muscular strength after taking shilajit supplements.",
          },
        ],
      },
      {
        title: "How to take Shilajit?",
        description:
          "You can find shilajit supplements at many online shops, pharmacies, drugstores, and health food stores. It's available as a dissolvable powder for drinks like water or milk and as a capsule.",
        subBody: [
          {
            title: "Dosage",
            description:
              "Shilajit isn't regulated by the Food and Drug Administration (FDA), meaning there's no set dosage deemed safe and effective. The dose administered in some clinical studies has been between 300–500 mg once per day.2 It's best to follow the dosing instructions from the manufacturer listed on the label if you are taking a shilajit supplement. You can also talk to a healthcare provider for guidance on finding a safe and effective dose.",
          },
        ],
      },
    ],
  },
  {
    id: 3,
    title: "Why Shilajit is good for you?",
    slug: "shilajit-benefits3",
    author: "Classsical Herbal Team",
    publishedDate: "July 24th, 2024",
    brief:
      "Shilajit, also known as mumiyo or mumie, is an organic mineral compound found in the Himalayan mountains and mountainous regions in Russia, Afghanistan, and elsewhere. This naturally occurring, blackish-brown resin contains over 40 minerals and substances, most notably fulvic acid.",
    image: "shilajit-benefits.png",
    body: [
      {
        title: "4 Benefit of Shilajit",
        description:
          "Shilajit, also known as mumiyo or mumie, is an organic mineral compound found in the Himalayan mountains and mountainous regions in Russia, Afghanistan, and elsewhere. This naturally occurring, blackish-brown resin contains over 40 minerals and substances, most notably fulvic acid.1",
        subBody: [
          {
            title: "1. Has Anti-Inflammatory and Antioxidant Effects",
            description:
              "Shilajit has strong anti-inflammatory and antioxidant effects. The compound is thought to promote greater longevity and have anti-aging properties in Ayurvedic medicine. These benefits can support overall health and reduce the risk of certain conditions.",
            bullet: {
              brief: "Research has shown that shilajit may also:",
              points: [
                "Increase cell turnover",
                "Potentially encourage the body to produce more collagen",
                "Slow down oxidative stress",
              ],
            },
          },
          {
            title: "2. May Prevent Bone Loss",
            image: "bone-loss.png",
            description:
              "Shilajit may have a positive effect on bone health. A study published in 2022 found postmenopausal women with osteopenia (low bone mineral density) who took shilajit supplements daily saw significant improvements in bone density after 11 months. The researchers theorized that this effect may be due to shilajit's ability to speed up cell turnover.",
          },
          {
            title: "3. Could Increase Male Fertility",

            description:
              "Shilajit may increase male fertility by boosting key hormones. Research has shown that taking 500 milligrams (mg) of shilajit per day can significantly increased dehydroepiandrosterone (DHEAS) and total testosterone levels.",
          },
          {
            title: "4. Might Improve Exercise Performance",
            image: "improve-exercise-performance.png",
            description:
              "A growing body of research indicates that shilajit can be helpful for exercise efforts. Some evidence suggests that athletes have less post-workout fatigue and are better able to reach peak muscular strength after taking shilajit supplements.",
          },
        ],
      },
      {
        title: "How to take Shilajit?",
        description:
          "You can find shilajit supplements at many online shops, pharmacies, drugstores, and health food stores. It's available as a dissolvable powder for drinks like water or milk and as a capsule.",
        subBody: [
          {
            title: "Dosage",
            description:
              "Shilajit isn't regulated by the Food and Drug Administration (FDA), meaning there's no set dosage deemed safe and effective. The dose administered in some clinical studies has been between 300–500 mg once per day.2 It's best to follow the dosing instructions from the manufacturer listed on the label if you are taking a shilajit supplement. You can also talk to a healthcare provider for guidance on finding a safe and effective dose.",
          },
        ],
      },
    ],
  },
  {
    id: 4,
    title: "Why Shilajit is good for you?",
    slug: "shilajit-benefits4",
    author: "Classsical Herbal Team",
    publishedDate: "July 24th, 2024",
    brief:
      "Shilajit, also known as mumiyo or mumie, is an organic mineral compound found in the Himalayan mountains and mountainous regions in Russia, Afghanistan, and elsewhere. This naturally occurring, blackish-brown resin contains over 40 minerals and substances, most notably fulvic acid.",
    image: "shilajit-benefits.png",
    body: [
      {
        title: "4 Benefit of Shilajit",
        description:
          "Shilajit, also known as mumiyo or mumie, is an organic mineral compound found in the Himalayan mountains and mountainous regions in Russia, Afghanistan, and elsewhere. This naturally occurring, blackish-brown resin contains over 40 minerals and substances, most notably fulvic acid.1",
        subBody: [
          {
            title: "1. Has Anti-Inflammatory and Antioxidant Effects",
            description:
              "Shilajit has strong anti-inflammatory and antioxidant effects. The compound is thought to promote greater longevity and have anti-aging properties in Ayurvedic medicine. These benefits can support overall health and reduce the risk of certain conditions.",
            bullet: {
              brief: "Research has shown that shilajit may also:",
              points: [
                "Increase cell turnover",
                "Potentially encourage the body to produce more collagen",
                "Slow down oxidative stress",
              ],
            },
          },
          {
            title: "2. May Prevent Bone Loss",
            image: "bone-loss.png",
            description:
              "Shilajit may have a positive effect on bone health. A study published in 2022 found postmenopausal women with osteopenia (low bone mineral density) who took shilajit supplements daily saw significant improvements in bone density after 11 months. The researchers theorized that this effect may be due to shilajit's ability to speed up cell turnover.",
          },
          {
            title: "3. Could Increase Male Fertility",

            description:
              "Shilajit may increase male fertility by boosting key hormones. Research has shown that taking 500 milligrams (mg) of shilajit per day can significantly increased dehydroepiandrosterone (DHEAS) and total testosterone levels.",
          },
          {
            title: "4. Might Improve Exercise Performance",
            image: "improve-exercise-performance.png",
            description:
              "A growing body of research indicates that shilajit can be helpful for exercise efforts. Some evidence suggests that athletes have less post-workout fatigue and are better able to reach peak muscular strength after taking shilajit supplements.",
          },
        ],
      },
      {
        title: "How to take Shilajit?",
        description:
          "You can find shilajit supplements at many online shops, pharmacies, drugstores, and health food stores. It's available as a dissolvable powder for drinks like water or milk and as a capsule.",
        subBody: [
          {
            title: "Dosage",
            description:
              "Shilajit isn't regulated by the Food and Drug Administration (FDA), meaning there's no set dosage deemed safe and effective. The dose administered in some clinical studies has been between 300–500 mg once per day.2 It's best to follow the dosing instructions from the manufacturer listed on the label if you are taking a shilajit supplement. You can also talk to a healthcare provider for guidance on finding a safe and effective dose.",
          },
        ],
      },
    ],
  },
];
